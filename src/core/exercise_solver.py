...
def _session_get(url: str, **kwargs):
    ...
    try:
        ...
        log_http_success("GET", url, resp.status_code, now_ms() - started, params=kwargs.get("params"))
        return resp
...
def _get_course_chapter(classroom_id: str, university_id: int) -> dict:
    ...
    try:
        ...
        log_http_success("GET", url, resp.status_code, now_ms() - started, params=params)
        return resp.json()
    except Exception as exc:
...
def _submit_answer(
    ...
    try:
        ...
        log_http_success("POST", url, resp.status_code, now_ms() - started)
    except Exception as exc:
...
def run_exercise_solver_session(selected_course: Optional[CourseSelection] = None):
    """
    ...
    if selected_course is None:
        classroom_id, university_id, course_info = select_course(_session_get)
    else:
